
import React, { Fragment, useState } from 'react';
import NavBar from '../components/navbar';
import Modal from '../components/modal';
const GDPR = (props) => {
  const [showModal, setShowModal] = useState(false);
  const [selectedItemId, setSelectedItemId] = useState(null);
  const handleLearnMoreClick = (itemId) => {
    setSelectedItemId(itemId);
    setShowModal(true);
  };
  return (
    <div>
      <Fragment>
        <NavBar />
        <section className="text-gray-600 body-font overflow-hidden">
          <div className="container px-5 py-24 mx-auto">
            {props.GDPR.data.map((item) => {
              return (
                <div className="-my-8 divide-y-2 divide-gray-60">
                  <div className="py-8 flex flex-wrap md:flex-nowrap">
                    <div className="md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col">
                      <span className="font-semibold title-font text-gray-700">PATTERN</span>
                      <span className="mt-1 text-gray-500 text-sm">{item.id}</span>
                    </div>
                    <div className="md:flex-grow">
                      <h2 className="text-2xl font-medium text-gray-900 title-font mb-2"></h2>
                      <p className="leading-relaxed">Descrizione: {item.attributes.Descrizione}</p>
                      <a
                        className="text-indigo-500 inline-flex items-center mt-4"
                        onClick={() => handleLearnMoreClick(item.id)}
                      >
                        Learn More
                        <svg className="w-4 h-4 ml-2" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                          <path d="M5 12h14"></path>
                          <path d="M12 5l7 7-7 7"></path>
                        </svg>
                      </a>
                    </div>
                  </div>
                  <div/>
                </div>
                
              );
            })}
          </div>
        </section>
        <Modal isVisible={showModal} onClose={() => setShowModal(false)} itemId={selectedItemId} />
      </Fragment>
    </div>
  );
};
export async function getServerSideProps(context) {
  let headers = {
    Authorization: "Bearer ",
  };
  let a = await fetch("http://localhost:1337/api/gdpr-knowledge-bases?populate=*", { headers: headers });
  let GDPR = await a.json();
  console.log(GDPR);
  return {
    props: { GDPR: GDPR },
  };
}
export default GDPR;

